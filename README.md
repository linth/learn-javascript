...
### 變數和資料型別 (Variable and Data Type)
- 變數宣告（var、let、const）、作用域
...
### 演算法 (Algorithm)
- 排序：Bubble, QuickSort, MergeSort
- 搜尋：Binary search
- 遍歷：DFS、BFS
- 時間複雜度 Big-O：O(1), O(n), O(log n), O(n log n)
- 空間複雜度 Big-O：O(1), O(n), O(log n), O(n log n)
